...
public static class IEEE488InstrumentTestInfo
{
    ...
    public static string ResetClearDevice( InstrumentId instrumentId, TimeSpan connectionTimeout )
    {
        ...
        using var lxiClient = new cc.isr.VXI11.Client.Vxi11Client();

        lxiClient.Connect( ipv4Address, "inst0" );

        string response = WriteDevice( lxiClient, command );
        _ = builder.Append( $"{command}: {response}\n" );

        command = "*CLS";
        if ( interQueryDelayMs > 0 ) Task.Delay( interQueryDelayMs ).Wait();
        response = WriteDevice( lxiClient, command );
        _ = builder.Append( $"{command}: {response}\n" );
    ...
    public static string QueryIdentity( InstrumentId instrumentId, TimeSpan connectionTimeout )
    {
        ...
        string response = QueryDevice( lxiClient, command, true );
        _ = builder.Append( $"a: {response}\n" );

        if ( interQueryDelayMs > 0 ) Task.Delay( interQueryDelayMs ).Wait();
        response = QueryDevice( lxiClient, command, true );
    ...
    private static string QueryDevice( cc.isr.VXI11.Client.Vxi11Client lxiClient, string command, bool trimEnd )
    {
        try
        {
            (bool success, string reply) = lxiClient.Query( $"{command}\n", 0, trimEnd );
            return reply;
        }
    ...
    /// <param name="command">      The command. </param>
    /// <returns>   A string. </returns>
    private static string WriteDevice( cc.isr.VXI11.Client.Vxi11Client lxiClient, string command )
    {
        try
        {
            int count = lxiClient.Write( $"{command}\n" );
