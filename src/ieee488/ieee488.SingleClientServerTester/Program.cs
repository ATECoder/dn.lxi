...
try
{
    using cc.isr.LXI.IEEE488.Mock.Ieee488SingleClientMockServer server = new();
    ...
    Logger.Writer.LogInformation( $"{nameof( cc.isr.LXI.IEEE488.Mock.Ieee488SingleClientMockServer )} is {(server.Running ? "running" : "idle")}  {DateTime.Now:ss.fff}" );
...
{
    string name = "unknown";
    if ( sender is cc.isr.LXI.IEEE488.Mock.Ieee488SingleClientMockServer ) name = nameof( cc.isr.LXI.IEEE488.Mock.Ieee488SingleClientMockServer );
